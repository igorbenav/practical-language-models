...
We can't access $f$ directly, we don't know it. What we have is a dataset $D = \{(x_1, y_1), (x_2, y_2), \ldots, (x_N, y_N)\}$: a collection of input-output pairs where each $y_i = f(x_i)$; these are examples of this function's behavior. $D$ is a finite subset of all possible input-output pairs, a sample from the vast space of everything $f$ could be asked to classify. The hope is that the patterns in this sample are representative of the whole: that what we learn from 60,000 digits generalizes to the millions we'll never see.

Our job is to use $D$ to find a function $g: X \to Y$ such that $g \approx f$. Not just on the examples we've seen, but on new inputs the machine has never encountered. That last part is what separates learning from memorization [@abu-mostafa2012learning].
...
Now we have a way to compare images. The simplest possible way to classify follows immediately: given a new image, compare it to every image in the training set. Find the most similar one. Use its label as the prediction. If the closest training image is a 7, predict 7. This is what we call **nearest neighbor** [@cover1967nearest].
...
If your model correctly classifies 9,200 out of 10,000 test digits, its accuracy is 92%. That's straightforward, but it hides an assumption: that the classes are roughly balanced. MNIST has about 7,000 images per digit, so accuracy is a fair measure, but if 90% of your emails are not spam, a model that always predicts "not spam" gets 90% accuracy without learning any relevant pattern. For MNIST this isn't a problem, but for other datasets, you'd want to look at per-class performance, which we'll do in the hands-on with the confusion matrix.
...
This also tells you when to use which: MSE is the right default when big errors are genuinely rare and mean something went wrong, but if your data has heavy outliers (house prices with a few mansions, sensor readings with occasional spikes), MSE overreacts to them and distorts the whole model. MAE doesn't let a few extreme values dominate the loss. When you pick a loss function, you're making a bet on what your error distribution looks like.

![Distributions vs Errors](../images/chapter1/figure-1-13-en.svg){width=75%}
...
![Underfitting, Good Fit, and Overfitting](../images/chapter1/figure-1-14-en.svg)
...
![The Generalization Gap](../images/chapter1/figure-1-15-en.svg){width=85%}
...
Remember to shuffle the data before splitting if it's necessary; MNIST comes sorted by digit, so a naive split puts all the 0s through 7s in training and all the 8s and 9s in test. The test score would be meaningless: the model never saw those digits during training. Shuffling ensures both sets are representative of the whole dataset.
...
We've been talking about handwritten digits the entire chapter. Now let's build the template matcher we described in the theory and see how it actually performs. We'll use MNIST [@lecun1998gradient]: 70,000 handwritten digits, each a 28×28 pixel grayscale image labeled 0 through 9. We'll use scikit-learn to download the data, but everything else is pure NumPy. No magic function calls. We build every piece ourselves.
...
$$\text{prediction} = \arg\min_{k \in \{0,...,9\}} \; d(x, \text{template}_k)$$
...
$$\text{accuracy}_d = \frac{\sum_{i : y_i = d} \mathbf{1}[\hat{y}_i = d]}{\sum_{i} \mathbf{1}[y_i = d]}$$
