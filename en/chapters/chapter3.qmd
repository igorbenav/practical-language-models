...
The loss function for negative sampling looks like this. For a target word $t$, a real context word $c$, and $K$ random negative words $n_1, \ldots, n_K$:

$$L = -\log \sigma(w_t \cdot w_c) - \sum_{k=1}^{K} \log \sigma(-w_t \cdot w_k)$$

The first term pushes the dot product up for the real pair (make the sigmoid close to 1). Each term in the sum pushes the dot product down for the random pairs (make the sigmoid close to 0). If this looks familiar, it should: it's the same sigmoid and binary cross-entropy from Chapter 2. Each word pair is a binary classification problem: "is this a real context pair or a random one?" Word2Vec isn't a new kind of algorithm. It's logistic regression applied to a clever prediction task, run billions of times until the vectors encode meaning.
...
We've talked about word embeddings as vectors that capture meaning, now let's work with real ones. We'll load pre-trained GloVe embeddings [@pennington2014glove] (Global Vectors for Word Representation). GloVe is closely related to Word2Vec but trains on global word co-occurrence statistics rather than local context windows; the resulting vectors exhibit the same properties: analogy arithmetic works, similar words cluster together, and biases carry over from the training text. We're using GloVe here because the pre-trained vectors are freely available and easy to load, but everything we explore in this section applies equally to Word2Vec embeddings. We'll compute similarity ourselves, run the analogy experiments, and investigate where the representation fails.
...
Now look at "happy" and "sad." You might expect them to be far apart (they're opposites), but they're quite similar (around 0.7). This tells you something important about what cosine similarity actually measures on distributional embeddings: **relatedness**, not substitutability. "Happy" and "sad" are antonyms, but they appear in almost identical contexts. You say "I feel happy" and "I feel sad." You say "She seemed happy" and "She seemed sad." The distributional hypothesis puts them close because they keep the same company. Cosine similarity captures how much two words have to do with each other, not whether you can swap one for the other without changing the meaning.
...
And the dot product of one vector against all 400,000 vectors at once is a matrix-vector multiply. We normalize the full matrix once up front so we don't redo it on every query:
...
`norms[:, np.newaxis]` reshapes the norms from a flat list into a column, so that when we divide vectors / norms[:, np.newaxis], each row gets divided by its own norm. This is the $\hat{v} = v / \|v\|$ step. Once normalized exists, every similarity query is a single matrix-vector multiply:
...
`normalized @ word_normalized` gives us 400,000 cosine similarities in a single matrix multiply. The `@` operator is matrix multiplication, and this is where NumPy is a lot better than doing it with standard Python: one line of code replaces 400,000 loops, it's done in parallel.
...
6. The `most_similar` function handles one word at a time. What if you need the neighbors of 100 words? Calling it in a loop works, but there's a faster way: compute all 100 queries at once with a single matrix multiply. `normalized[query_indices]` grabs the rows for your query words, giving you a matrix of shape `(100, 50)`. Multiply it by the full normalized matrix transposed: `(normalized @ normalized[query_indices].T)` gives you a `(400000, 100)` matrix of all pairwise similarities in one operation. Time this batch version against calling `most_similar` in a loop for 100 random words. How much faster is it? This is the core trick behind vector databases, which we'll build with in Chapter 10.
